/**
 * WebSocket transport — copied from apps/web/src/wsTransport.ts
 * Simplified for mobile: no desktopBridge, direct URL only.
 */
import {
  type WsPush,
  type WsPushChannel,
...
export class WsTransport {
  ...
  private handleMessage(raw: unknown) {
    ...
    if (!isWebSocketResponseEnvelope(message)) return;
  ...
  private send(encodedMessage: string) {
    ...
    try {
      ...
      // Swallow: message is queued for retry on reconnect
    }
  }
