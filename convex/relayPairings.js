...
export const savePairing = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_pair", (q) => q.eq("deviceId", args.deviceId).eq("pairedDeviceId", args.pairedDeviceId))
            .first();
        if (existing) {
...
export const removePairing = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const reverse = await ctx.db
            .query("devicePairings")
            .withIndex("by_pair", (q) => q.eq("deviceId", args.pairedDeviceId).eq("pairedDeviceId", args.deviceId))
            .first();
        if (reverse)
